// == Nezha Dashboard IP Ping 按钮（按钮显示在 IP 下方，安全版）==
(function(){
  'use strict';
  ...
  function createPingButton(ip, isIPv4){
    ...
    a.textContent = isIPv4 ? 'Ping IPv4' : 'Ping IPv6';
    a.href = isIPv4 
  ...
  function appendPingButtons(cell){
    ...
    if (v4){
      wrap.appendChild(createPingButton(v4, true));
    }
    if (v6){
      wrap.appendChild(createPingButton(v6, false));
    }

    if (wrap.children.length > 0){
      cell.appendChild(wrap); // ✅ 只追加，不修改原IP
    }
