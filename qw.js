// == Nezha Dashboard IP Ping 按钮（按钮在 IP 下方）==
(function(){
  'use strict';
  ...
  style.textContent = `
    .nezha-ping-btn {
      ...
      margin-top:4px; border-radius:6px; cursor:pointer; user-select:none;
  ...
  function appendPingButtons(cell){
    ...
    if(wrap.children.length > 0){
      // 在 cell 内的第一个子节点（IP 文本）后面插入
      const firstNode = cell.firstChild;
      if(firstNode){
        firstNode.parentNode.insertAdjacentElement('afterend', wrap);
      }
    }
