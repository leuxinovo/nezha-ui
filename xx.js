// == Nezha Dashboard IP Ping 按钮（按钮在 IP 下方，区分 v4/v6）==
(function(){
  'use strict';
  ...
  style.textContent = `
    .nezha-ping-btn {
      ...
      margin:2px 4px 0 0; border-radius:6px; cursor:pointer; user-select:none;
  ...
  function createPingButton(ip, isIPv4){
    ...
    a.textContent = isIPv4 ? 'Ping v4' : 'Ping v6';  // ✅ 区分按钮文字
    a.href = isIPv4 
  ...
  function appendPingButtons(cell){
    ...
    if(v4 && !cell.querySelector(`[data-ip="${v4}"]`)){
      const btn4 = createPingButton(v4, true);
      btn4.dataset.ip = v4;
      wrap.appendChild(btn4);
    }

    if(v6 && !cell.querySelector(`[data-ip="${v6}"]`)){
